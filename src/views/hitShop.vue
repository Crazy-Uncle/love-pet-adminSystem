...
import { ref } from 'vue'
import { getShop, addRepertory, delShop, searchShop } from '@/utils/get'
...
const search = () => {
  searchShop({ name: input.value }).then((res) => {
    if (res.data.status === 200) {
      ...
      ElMessage({ message: '未找到此商品', type: 'warning', duration: 1000 })
    }
  })
}
...
const add = (val: v) => {
  ...
  ElMessageBox.prompt('填写要修改的库存数量', '增加库存', {
    ...
    .then(({ value }) => {
      if (parseInt(value) + val.repertory < 0) {
        ElMessage({
          message: '库存数不能小于0',
          type: 'warning',
          duration: 700
        ...
        addRepertory({ id: val.id, num: parseInt(value) }).then((res) => {
          if (res.data.status === 200) {
            ElMessage({ message: '修改成功', type: 'success', duration: 700 })

            log({
              username: localStorage.getItem('username'),
              log: '在上架商品页面对：' + val.name + '修改了库存信息'
            }).then((res) => {
              if (res.data.status === 200) {
                // console.log('记录成功')
              } else {
...
// 删除商品
const del = (val: d) => {
  delShop({ id: val.id }).then((res) => {
    if (res.data.status === 200) {
      ElMessage({ message: '删除成功', type: 'success', duration: 500 })

      log({
        username: localStorage.getItem('username'),
        log: '在上架商品页面对：' + val.name + '进行了删除操作'
      }).then((res) => {
        if (res.data.status === 200) {
          // console.log('记录成功')
        } else {
...
// 修改商品基本信息
const infoChange = () => {
  changeShop(infoValue.value).then((res) => {
    if (res.data.status === 200) {
...
const changCover = () => {
  ...
  cover(form)
    .then((res) => {
      if (res.data.status === 200) {
        changeImg({ id: infoValue.value.id }).then((res) => {
          ElMessage({
            message: '更换商品封面成功',
            type: 'success',
            duration: 1000
...
const changeSlid = () => {
  ...
  if (fileLists.value.length < 1) {
    ...
    avatar(formData)
      .then((res) => {
        if (res.data.status === 200) {
          changeSild({ id: infoValue.value.id }).then((res) => {
            if (res.data.status === 200) {
              ElMessage({
                message: '更换商品轮播图成功',
                type: 'success',
...
const changeL = () => {
  ...
  if (lifeList.value.length < 1) {
    ...
    life(formLife)
      .then((res) => {
        if (res.data.status === 200) {
          changeLife({ id: infoValue.value.id }).then((res) => {
            if (res.data.status === 200) {
              // console.log(res)
